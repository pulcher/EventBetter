...
public static async Task<HttpResponseMessage> Run(HttpRequestMessage req, CloudTable currentDevices, TraceWriter log)
{
        ...
        await currentDevices.ExecuteQuerySegmentedAsync(query, null))
    {
        if (device.Name == name)
        {
            var httpQuery = $"https://api.particle.io/v1/devices/{device.DeviceId}/acknowledge?access_token={access_token}";
    ...
    return !resetSent
        ...
        : req.CreateResponse(HttpStatusCode.OK, "Responding to " + name);
}
